const inventors = [
  ...
  // 1. Filter the list of inventors for those who were born in the 1500's
    let result = inventors.filter(function(investor) {
  ...
  let birthOrder = inventors.sort(function(a, b) {
  ...
  let totalYears = inventors.reduce(function(a, inventor) {
  ...
  // 5. Sort the inventors by years lived
    ...
    let yearsLived = inventors.sort(function(a, b) {
        let lastMan = a.passed - a.year;
        let firstMan = b.passed - b.year;
         if (lastMan > firstMan) return -1;
         else 1;
    });
  ...
  // Sort the people alphabetically by last name
    ...
    let alpha = people.sort(function(first, last) {
  ...
  const data = ['car', 'car', 'truck', 'truck', 'bike', 'walk',
   ...
   let transport = data.reduce(function(obj, item) {
    if (!obj[item]) {
        obj[item] = 0;
    }
    obj[item]++;
